...
onmessage = function(event){
	...
	for(var i = 0; i < pointAttributes.attributes.length; i++){
		...
		if(pointAttribute.name === Potree.PointAttribute.POSITION_CARTESIAN.name){
			...
			for(var j = 0; j < numPoints; ++j){
				colors[3*j+0] = (cv.getUint16(offset + j*pointSize + 0) % 255) / 255;
				colors[3*j+1] = (cv.getUint16(offset + j*pointSize + 2) % 255) / 255;
				colors[3*j+2] = (cv.getUint16(offset + j*pointSize + 4) % 255) / 255;
			}
